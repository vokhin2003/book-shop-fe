...
export const changePasswordAPI = (
  ...
  return axios.post<IBackendRes<null>>("api/v1/auth/change-password", {
    id,
    oldPassword,
    newPassword,
  });
...
export const updateUserAPI = (
  ...
  return axios.put<IBackendRes<IUser>>(`/api/v1/users/${id}`, {
    fullName,
    email,
    address,
    phone,
...
export const updateUserInfoAPI = (
  ...
  return axios.put<IBackendRes<IUser>>("/api/v1/users/info", {
    id,
    fullName,
    phone,
    avatar,
...
export const updatePermissionAPI = (
  ...
  return axios.put<IBackendRes<IPermission>>(`/api/v1/permissions/${id}`, {
    name,
    path,
    method,
    module,
...
export const updateRoleAPI = (
  ...
  return axios.put<IBackendRes<IRole>>(`/api/v1/roles/${id}`, {
    name,
    description,
    permissionIds,
  });
...
export const addToCartAPI = (bookId: number, quantity: number) => {
  return axios.post<IBackendRes<ICartItem>>("/api/v1/carts", {
    bookId,
    quantity,
  });
};
...
export const updateCartAPI = (bookId: number, quantity: number) => {
  return axios.put<IBackendRes<ICartItem>>("/api/v1/carts", {
    bookId,
    quantity,
  });
};
...
export const fetchOrderAPI = (query: string) => {
  return axios.get<IBackendRes<IModelPaginate<IOrder>>>(
    `/api/v1/orders?${query}`
  );
};
...
export const getTransactionStatusAPI = (transactionId: string) => {
  return axios.get<IBackendRes<IPaymentResult>>(
...
export const createPaymentUrlAPI = (paymentData: {
  ...
  return axios.post<IBackendRes<IPaymentResponse>>(
    `/api/v1/payments`,
    paymentData
  );
};
...
export const cancelOrderAPI = (orderId: number) => {
  return axios.put<IBackendRes<{ orderId: number; status: EOrderStatus }>>(
...
export const saveDeviceTokenAPI = (body: {
  userId: number;
  deviceToken: string;
  deviceType: string;
}) => {
  return axios.post<IBackendRes<null>>("/api/v1/users/device-token", body);
};

export const removeDeviceTokenAPI = (body: {
  userId: number;
  deviceToken: string;
  deviceType: string;
}) => {
