...
const LandingPage = () => {
    ...
    const testimonials: { quote: string; author: string }[] = [
        { quote: "Sách mới, đóng gói kỹ, giao siêu nhanh!", author: "Anh Minh – Hà Nội" },
        { quote: "Giá hợp lý, nhiều mã giảm. Sẽ ủng hộ dài dài!", author: "Chị Linh – TP.HCM" },
        { quote: "CSKH phản hồi nhanh, xử lý đổi trả rất ổn.", author: "Bạn Khánh – Đà Nẵng" },
        { quote: "Danh mục đa dạng, dễ tìm sách yêu thích.", author: "Bạn Hương – Hải Phòng" },
        { quote: "Thanh toán VNPAY tiện, nhận hàng đúng hẹn.", author: "Anh Nam – Cần Thơ" },
        { quote: "Sách chính hãng, mực in rõ, giấy dày đẹp.", author: "Chị Vy – Bình Dương" },
    ];
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (isMounted && bookRes?.data?.result) setBestSellers(bookRes.data.result);
                if (isMounted && cateRes?.data?.result) setCategories(cateRes.data.result);
                const newRes = await fetchBookAPI("page=1&size=8&sort=updatedAt,desc");
                if (isMounted && newRes?.data?.result) setNewArrivals(newRes.data.result);
            } finally {
        ...
        <div className={styles["landing-root"]}>
            <section className={styles.hero}>
                <div className={styles["hero-inner"]}>
                    <Title className={styles["hero-title"]}>Khám phá kho tàng sách cho mọi đam mê</Title>
                    <Paragraph className={styles["hero-subtitle"]}>
                        Từ sách kỹ năng, văn học, thiếu nhi đến học thuật. Giao hàng nhanh, thanh toán an toàn, ưu đãi mỗi ngày.
                    </Paragraph>
                    <Space size="middle" wrap>
                        <Button size="large" type="primary" onClick={() => navigate("/shop")}>Bắt đầu mua sắm</Button>
                        <Button size="large" onClick={() => document.getElementById("section-bestseller")?.scrollIntoView({ behavior: "smooth" })}>Xem sách nổi bật</Button>
                    </Space>
                </div>
                ...
                <Row gutter={[24, 24]} justify="center">
                    <Col xs={24} md={8}>
                        <div className={styles["feature-item"]}>
                            <RocketTwoTone twoToneColor="#1677ff" className={styles["feature-icon"]} />
                            <Title level={4}>Giao hàng nhanh</Title>
                            <Paragraph>Nhận hàng trong 24-48h tại nội thành. Theo dõi đơn hàng thời gian thực.</Paragraph>
                        </div>
                    </Col>
                    <Col xs={24} md={8}>
                        <div className={styles["feature-item"]}>
                            ...
                            <Paragraph>Hỗ trợ VNPAY và nhiều phương thức. Mã hóa và xác thực bảo mật.</Paragraph>
                        </div>
                    </Col>
                    <Col xs={24} md={8}>
                        <div className={styles["feature-item"]}>
                            <ThunderboltTwoTone twoToneColor="#faad14" className={styles["feature-icon"]} />
                            <Title level={4}>Ưu đãi mỗi ngày</Title>
                            <Paragraph>Flash sale, mã giảm giá và quà tặng thành viên thân thiết.</Paragraph>
                        </div>
                    </Col>
                </Row>
            </section>
            ...
            <section className={styles.categories}>
                ...
                <Row gutter={[16, 16]}>
                    {categories.map((c, idx) => {
                        const gradients = [styles.g1, styles.g2, styles.g3, styles.g4, styles.g5, styles.g6];
                        ...
                        return (
                            <Col xs={12} sm={8} md={6} lg={4} key={c.id}>
                                ...
                                >
                                    ...
                                    <Title level={5} className={styles["cate-title"]}>{c.name}</Title>
                                    {c.description && (
                                        <Paragraph className={styles["cate-desc"]}>{c.description}</Paragraph>
                                    )}
                                </Card>
                            </Col>
                        );
            ...
            <section id="section-bestseller" className={styles.bestseller}>
                <div className={styles["section-head"]}>
                    ...
                    <Paragraph>Những tựa sách được yêu thích nhất bởi cộng đồng độc giả</Paragraph>
                </div>
                                ...
                                {bestSellers.map((b) => (
                                    <SwiperSlide key={b.id}>
                                        <Card
                                            ...
                                            cover={<img alt={b.title} src={b.thumbnail} className={styles["book-thumb"]} />}
                                            onClick={() => navigate(`/book/${encodeURIComponent(b.title)}?id=${b.id}`)}
                                        >
                                            ...
                                            <Text strong>
                                                {new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(b.price ?? 0)}
                                            </Text>
                                        </Card>
                                    </SwiperSlide>
                                ))}
                            ...
                            <Col xs={12} sm={8} md={6} lg={6} key={b.id}>
                                ...
                                >
                                    <div className={styles["thumb-wrap"]}>
                                        <img alt={b.title} src={b.thumbnail} className={styles["book-thumb"]} />
                                    </div>
                                    <div className={styles["book-body"]}>
                                        <Title level={5} className={styles["book-title"]}>{b.title}</Title>
                                        <Text strong>
                                            {new Intl.NumberFormat("vi-VN", { style: "currency", currency: "VND" }).format(b.price ?? 0)}
                                        </Text>
                                    </div>
            ...
            <section className={styles.partners}>
                ...
                {(() => {
                    const partnerLogos: { src: string; alt: string }[] = [
                        ...
                        { src: "/partners/nguyenvancu.png", alt: "Nguyễn Văn Cừ" },
            ...
            <section className={styles.newsletter}>
                <div className={styles["newsletter-inner"]}>
                    ...
                    <Paragraph>Đăng ký email để không bỏ lỡ các đợt giảm giá hấp dẫn</Paragraph>
                    <Form
                        layout="inline"
            ...
            <section className={styles.cta}>
                ...
                <Paragraph>Khám phá ngay hàng ngàn cuốn sách với giá tốt nhất hôm nay.</Paragraph>
                <Button size="large" type="primary" onClick={() => navigate("/shop")}>Khám phá cửa hàng</Button>
            </section>
        </div>
    );
};
